...
export class StihlProductImageNormalizer extends ProductImageNormalizer{
    ...
    convert(source: Occ.Product, target?: StihlProduct): StihlProduct {
       ...
       normalizeStihlImages(source: any[]): StihlImage[] {
        ...
        if (source) {
          for (const image of source) {
              console.log(image);
              images.push({atlText: image.altText, 
                mamId: image.mamId, 
                mamType: image.mamType,
                url: this.normalizeStihlImageUrl(image.mamId)
            })
       ...
       * if none provided.
       */
      private normalizeStihlImageUrl(url: string): string {
        if (new RegExp(/^(http|data:image|\/\/)/i).test(url)) {
          return url;
        }
        return "https://www.stihl.fr/content/dam/stihl/mam/modell-landscape/"+url+".jpg";
